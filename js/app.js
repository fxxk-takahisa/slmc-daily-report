...
class DailyReportApp {
    ...
    generateReport() {
        ...
        // 日次レポートと同じフォーマットで各日のデータを出力
        const reportText = this.buildMultipleDailyReportsText(title, reports);
        document.getElementById('reportOutput').textContent = reportText;
        this.currentReportText = reportText;
    ...
    // 複数日の日次レポート生成（週次・月次・累計用）
    buildMultipleDailyReportsText(title, reports) {
        let text = `${'='.repeat(60)}\n`;
        text += `${title}\n`;
        ...
        // 各日のデータを日次レポートと同じフォーマットで出力
        reports.forEach((report, index) => {
            if (index > 0) {
                text += `\n${'='.repeat(60)}\n\n`;
            }
            text += this.buildDailyReportText(report);
        });
